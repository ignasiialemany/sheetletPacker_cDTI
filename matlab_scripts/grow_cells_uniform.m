function grow_cells_uniform(cells, sheetlet, resx, resy, targetECV, filename)
    ...
    centroid_y = (max(y_sheetlet) - min(y_sheetlet)) * 0.5 + min(y_sheetlet);
     ...
     [labeledCells, numCells] = bwlabel(bw_cells_resized);
     ...
     boundaries = bwboundaries(bw_cells_resized);
     for i = 1:numCells
         ...
         for j = overlappingCells
             if i ~= j && potentialOverlap(i, j)
                 poly1 = boundaries{i};
                 poly2 = boundaries{j};
                 mask1 = poly2mask(poly1(:, 2), poly1(:, 1), size(bw_cells_resized, 1), size(bw_cells_resized, 2));
                 mask2 = poly2mask(poly2(:, 2), poly2(:, 1), size(bw_cells_resized, 1), size(bw_cells_resized, 2));
     ...
     filepath = "../matlab_outputs/before_thicken_" + filename + ".mat";
     display(filepath);
    ...
    n_to_remove = numCells - n_poly_approx;       
    disp(n_to_remove);
    ...
    filepath = "../matlab_outputs/after_thicken_" + filename + ".mat";
    display(filepath);
    save(filepath,"polygons");
end
